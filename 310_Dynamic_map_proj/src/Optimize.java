...
public class Optimize {
	...
	*/
	...
	final int[] UPSIDE_ELEVATOR_B1_RUNNING_INFO = {1,3,5,8,11};
	...
	*/
	...
	Optimize(Coursenum coursenum){
		this.coursenum = coursenum;
		this.Elevator = new float[coursenum.DAY_NUM][coursenum.ROW_NUM][this.ELEVATOR_NUM];
		this.totalPeopleNum = new float[coursenum.DAY_NUM][coursenum.ROW_NUM];

		this.runningElevator = new ArrayList[coursenum.DAY_NUM][coursenum.ROW_NUM][this.ELEVATOR_NUM];
		...
		// initialize Elevator
		this.magicfloor = new ArrayList[coursenum.DAY_NUM][coursenum.ROW_NUM];
		for(int i=0; i<coursenum.DAY_NUM; i++) {
			for(int j=0; j<coursenum.ROW_NUM; j++){
	...
	public void CalMagicFloor() {
		for(int day=0; day<coursenum.DAY_NUM; day++) {
			for(int row=0; row<coursenum.ROW_NUM; row++) {
				float floating_population = coursenum.getFloatingPopulation(day, row);
				//  total floating populaton is bigger than x
				if(floating_population >= x) {
					for(int column=0; column < 7; column++) {
						// peoplenum is bigger than y%
						if(floating_population * y <= coursenum.getPeopleNum(day, row, column)) {
				...
				// but floating polulation is bigger than y
					...
					for(int column=0; column < 7; column++){
						if(coursenum.getPeopleNum(day, row, column) >= z )
							magicfloor[day][row].add(floor[column]);
					}
				}
			}
	...
	public void Fileprint() {
		for(int day=0; day<coursenum.DAY_NUM; day++) {
			...
			for(int row=0; row<coursenum.ROW_NUM; row++) {
				...
				for(int column=0; column<coursenum.FLOOR_NUM; column++) {
					System.out.print("\t"+coursenum.getPeopleNum(day, row, column) + " ");
				}
				System.out.println();
			}
			System.out.print("\n\n");
	...
	public void Elevator_ruuningInfo(){
		...
		runningElevator_InfoA_Elevator(2, UPSIDE_ELEVATOR_A3_RUNNING_INFO);
		runningElevator_InfoA_Elevator(3, UPSIDE_ELEVATOR_A4_RUNNING_INFO);

		// B area
		runningElevator_InfoB_Elevator(4, UPSIDE_ELEVATOR_B1_RUNNING_INFO, EVEN);
		runningElevator_InfoB_Elevator(6, UPSIDE_ELEVATOR_B3_RUNNING_INFO, ODD);
		runningElevator_InfoNormal(5, ALL_STOP_ELEVATOR_B2_RUNNING_INFO);
		runningElevator_InfoNormal(7, MULTIPLES_OF_3_ELEVATOR_B4_RUNNING_INFO);

		runningElevator_InfoC_Elevator(8, ODD_ELEVATOR_C1_RUNNING_INFO);
		runningElevator_InfoC_Elevator(9,EVEN_ELEVATOR_C2_RUNNING_INFO);
	}
	...
	public void upsideElevator_Aarea(int number){
		...
		* 1 -> magic floor -> 1
		*/
		for(day=0; day<coursenum.DAY_NUM; day++){
			for(row =0; row<coursenum.ROW_NUM; row++){
	...
	*/
	...
	public void avgPeoplenum(){
		...
		this.avgPeoplenum = (int)Math.ceil( temp / (coursenum.DAY_NUM * coursenum.ROW_NUM) );
	...
	public void runningElevator_InfoA_Elevator(int elevator_num, int[] elevator_runninginfo){
		for(int day=0; day< coursenum.DAY_NUM; day++){
			for(int row=0; row < coursenum.ROW_NUM; row++){
				//magic floor existing
				if(!magicfloor[day][row].isEmpty()){
					this.runningElevator[day][row][elevator_num].add(1);
					for(int index = 0; index < magicfloor[day][row].size(); index++)
	...
	public void runningElevator_InfoB_Elevator(int elevator_num, int[] elevator_runninginfo, int num){
		int flag = 0;
		...
		for(int day=0; day< coursenum.DAY_NUM; day++){
			for(int row=0; row < coursenum.ROW_NUM; row++){
				//magic floor existing
				if(!magicfloor[day][row].isEmpty()){
					this.runningElevator[day][row][elevator_num].add(1);	//1 floor
					for(int index = 0; num <=12; index++){
						for(int j =0;  j < magicfloor[day][row].size(); j++){
							if(magicfloor[day][row].get(j) == num)	//except magic floor
								flag = 1;
						}
						if(flag != 1)
							this.runningElevator[day][row][elevator_num].add(num);
						num +=2;
						flag = 0;
					}
				}
	...
	public void runningElevator_InfoC_Elevator(int elevator_num, int[] elevator_runninginfo){
		for(int day=0; day< coursenum.DAY_NUM; day++){
			for(int row=0; row < coursenum.ROW_NUM; row++){
				//magic floor existing
				if(!magicfloor[day][row].isEmpty()){
					this.runningElevator[day][row][elevator_num].add(-3);	//B3 floor
					this.runningElevator[day][row][elevator_num].add(1);	//B3 floor
					for(int index = 0; index < magicfloor[day][row].size(); index++){
						if(magicfloor[day][row].get(index) != index)	//except magic floor
							this.runningElevator[day][row][elevator_num].add(magicfloor[day][row].get(index));
					}
				}
				else{
